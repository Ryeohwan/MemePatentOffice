...
const MemeSituationInput: React.FC = () => {
  ...
  const changeHandler = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (situationState === 0) dispatch(memeUploadActions.setSituationState(-1));
  };
  ...
  return (
    <div className={styles.situationInputContainer}>
      <div className={`${styles.inputContainer} ${situationState === 0 && styles.errorBox}`}>
        <textarea
      ...
      <div className={styles.errorMsg}>
        {situationState === 0 && <p>유해성 검사를 통과하지 못했습니다.</p>}
      </div>
    </div>
  );
};
