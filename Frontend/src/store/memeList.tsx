...
export const getMemeNewListAxios = (input: string) => {
  return async (dispatch: any) => {
    const sendRequest = async () => {
      dispatch(memeListActions.isMemeNewListLoading(true));

      const response = await axios.get(
        `${process.env.REACT_APP_HOST}/api/mpoffice/meme/search` + new URLSearchParams({
            search: input
        })
      );
    };
